...
namespace Saleos.Test.Controller.AdminControllerTest
{
    public class DeleteCategoryTest : HomePageControllerTest
    {
        ...
        public async Task DeleteCategory_CategoryIdIsOutOfRange_GetNotFound(int categoryId)
        {
        ...
        [InlineData(1)]
        [InlineData(2)]
        public async Task DeleteCategory_ValidCategoryId_DeleteTagInDatabase(int categoryId)
        {
            using var context = getContext();
            var articleServices = new ArticleServicesImpl(context);
            var controller = new AdminController(articleServices);
            ...
            Assert.False(await context.Categories.AnyAsync( x => x.Id == categoryId));
        }
    }
}
